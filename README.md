...
> 两种服务都是「按运单号查询」。免费额度个人日常够用；用量大需付费升级。
> 凭据只存在本机，填到登录页对应服务商后即可使用。
...
4. 在「控制台 → 账号信息/密钥」查看两项凭据：
   - **customer**（企业ID / 授权码）
   - **key**（授权 key）
5. 软件登录页选「**快递100 API**」，填入 customer 与 key → 保存并进入。

- 官方实时查询接口文档：<https://api.kuaidi100.com/document/5f0ffb5ebc8da837cbd8aefc.html>
- 不填 key 时，本软件自动用「免费公开查询」兜底（部分快递需手机尾号）。
...
4. 「用户中心 → 我的信息/密钥管理」查看两项凭据：
   - **EBusinessID**（用户ID）
   - **AppKey**（密钥）
5. 软件登录页选「**快递鸟 API**」，填入 EBusinessID 与 AppKey → 保存并进入。
